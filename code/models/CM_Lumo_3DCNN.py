...
VERSION = '0.0.3'
MODELTITLE = 'Predicting Alpha unocc. energies'
MODELTYPE = '3D CNN CPU'
MAXHEAVYATOMS = 20
...
data = loaddata(os.path.join(PATH, os.pardir, os.pardir, 'data', 'CNOS_sub1_10to20_10k', 'training', 'homo_lumo'))
...
train_labels = train['lumo'].to_numpy(dtype=np.float).tolist()
train_labels = np.array(train_labels)
log.info('Converting test labels to array...')
test_labels = test['lumo'].to_numpy(dtype=np.float).tolist()
